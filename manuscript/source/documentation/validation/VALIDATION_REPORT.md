...
- The regret-constrained medoid differs from the canonical highest-score candidate in 11 banks; all 11 have positive eligible-pool mean-Jaccard gain; median gain 0.04285714285714291.
...
- All Figure 6 and S16--S18 PDFs are byte-identical to their independently audited analysis outputs; only their printed Supplementary numbers changed during integration.
...
- Full details are recorded in `SCIENTIFIC_REFINEMENT_REPORT.md`.

## Consistency and narrative audit

- Relocated the five-candidate/top-three-regret note from immediately before Supplementary Table S1 to the archived CGGA locking-sensitivity table that it qualifies.
- Corrected the full-feature Bayesian reference in Supplementary Table S20 from Figure S4 to Figure S23.
- Added the fully nested TCGA GBM/LGG preprocessing and outer-test boundary to Supplementary Table S25.
- Standardized scientific selection terminology around `regret-constrained medoid` and `legacy top-three medoid`; configuration-level objective comparisons are identified separately as corrected-objective analyses.
- Recast the Abstract around the finite-bank locking contribution, expanded the Introduction to situate model multiplicity, multiobjective evolutionary feature selection, nested selection bias, and stability estimands, and consolidated the Discussion claim boundary.
- Elevated the distinction between stable signature cardinality and unstable feature identity as a substantive empirical finding rather than repeating it only as a limitation.
...
- A clean submission archive was built from the authoritative manuscript tree.
