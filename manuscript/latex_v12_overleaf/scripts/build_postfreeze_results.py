...
def build_amp_s21() -> dict[str, str]:
    ...
    for panel, comparator, title in [
        ...
        ax.axvline(0, color="#263746", lw=0.8)
    ...
    handles = [
        ...
        plt.Line2D([], [], marker="o", color="#263746", markerfacecolor="white", linestyle="", label="Small cap"),
        plt.Line2D([], [], marker="s", color="#263746", markerfacecolor="#263746", linestyle="", label="Reference cap"),
    ]
...
def build_nested_s23() -> dict[str, str]:
    ...
    folds = np.arange(1, 6)
    axes[0].plot(folds, selections["legacy_selected_run_id"], "o-", color="#777777", label="Legacy")
    axes[0].plot(folds, selections["upgraded_selected_run_id"], "o-", color="#2f6fb0", label="Current")
    ...
    axes[1].plot(folds, old_fold.set_index("outer_fold").loc[folds, "outer_auroc"], "o-", color="#777777")
    axes[1].plot(folds, [new_fold[x] for x in folds], "o-", color="#2f6fb0")
    ...
    for offset, (summary, color, label) in enumerate([(old_summary, "#777777", "Legacy"), (new_summary, "#2f6fb0", "Current")]):
    ...
    handles = [plt.Line2D([], [], marker="o", color="#777777", label="Legacy top-3 lock"), plt.Line2D([], [], marker="o", color="#2f6fb0", label="Current regret lock")]
