...
def main() -> None:
    ...
    checks.append("Figure S24 sensitivity axes match the frozen design")
    ...
    if 'width="170mm"' not in svg or 'height="67mm"' not in svg:
        raise AssertionError("Figure S24 canvas is not 170 x 67 mm")
    if "grid" in svg.lower():
        raise AssertionError("Figure S24 contains an unexpected grid marker")
    for panel in ("a)", "b)", "c)"):
        if panel not in svg:
            raise AssertionError(f"Missing panel label {panel}")
    checks.append("Figure S24 PDF/SVG/PNG hashes, 170-mm canvas, and grid-free style pass")
    ...
    if main_text.count("Supplementary Fig.~S24") < 3:
        raise AssertionError("Figure S24 is not cited in Results, Discussion, and Methods")
    if main_text.count("Supplementary Table~S25") < 3:
        raise AssertionError("Table S25 is not cited in Results, Discussion, and Methods")
    if supplementary.count("\\label{fig:supp24}") != 1:
        raise AssertionError("Supplementary Figure S24 label is missing or duplicated")
    if table.count("\\label{tab:supp25}") != 1 or "q95 denotes" not in table:
        raise AssertionError("Supplementary Table S25 label or q95 definition failed")
    ...
    checks.append("Results, Discussion, and Methods citations plus S24/S25 labels pass")

    grid_manifest = json.loads(
        (ROOT / "FIGURE_GRIDFREE_REPRODUCIBILITY_MANIFEST.json").read_text(encoding="utf-8")
    )
    if "figure_s24" not in grid_manifest["figures"]:
        raise AssertionError("Figure S24 is absent from the reproducibility manifest")
    checks.append("figure and source-data manifests include Supplementary Figure S24")
