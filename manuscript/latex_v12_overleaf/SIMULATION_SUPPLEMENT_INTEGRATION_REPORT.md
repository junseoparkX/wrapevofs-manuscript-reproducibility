...
The completed CPU-only candidate-bank locking simulation is integrated into the existing V12 manuscript without creating a new version. It appears as Supplementary Figure S24 and Supplementary Table S25 and is cited in the main Results, Discussion, and Methods. No main figure was replaced and no Figure 6 was added.
...
- Raw checkpoints retain `duplicate_multiplicity`, but the frozen scenario-summary CSV does not aggregate the selected mask's exact multiplicity distribution. The manuscript therefore reports the duplicate-handling policy and duplicate-rate sensitivity design but makes no numerical claim about that unaggregated distribution; no rerun is required for the presented S24/S25 claims.
...
The output is available as PDF, SVG, and 300-dpi PNG. The figure is generated from three compact frozen aggregate CSVs by `scripts/build_locking_simulation_s24.py`; browser/Poppler rendering is recorded separately. It is placed on Supplementary page 28 and has no clipped axis, legend, label, or caption.
...
- `scripts/validate_locking_simulation_s24_integration.py` passed every frozen-value, workload, feasibility, decision-path, hash, style, label, and citation check. Its machine-readable result is `supplementary_data/locking_rule_simulation/INTEGRATION_VALIDATION.json`.
